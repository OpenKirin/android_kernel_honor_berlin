...
int charger_dsm_report (int err_no,int *val)
{
    ...
    if(get_index(err_no) > (int)(sizeof(err_count)/sizeof(struct charger_dsm)-1))
    {
         return -1;
    }
...
**********************************************************/
static int charge_rename_charger_type(enum hisi_charger_type type, struct charge_device_info *di)
{
    bool ret = TRUE;
    switch(type)
    {
        ...
        case CHARGER_TYPE_UNKNOWN:
            if(CHARGER_REMOVED == di->charger_type || CHARGER_TYPE_USB == di->charger_type)
            {
...
static void select_ico_current(struct charge_device_info *di)
{
    ...
    if((!di->ops) || (!di->ops->turn_on_force_ico)){
        return;
    }
    ...
    if(ico_changed){
        output.ico_current = di->input_current;
        ico_changed = FALSE;
    }
    di->ops->turn_on_force_ico(&input, &output);

    di->input_current = output.input_current;
    di->sysfs_data.ico_current = output.ico_current;
}
...
static int charge_usb_notifier_call(struct notifier_block *usb_nb, unsigned long event, void *data)
{
    ...
    charge_wake_lock();
    filter_flag = charge_rename_charger_type((enum hisi_charger_type)event, di);
    if(filter_flag)
    {
        return NOTIFY_OK;
    }
...
static struct charge_sysfs_field_info charge_sysfs_field_tbl[] = {
    ...
    CHARGE_SYSFS_FIELD_RO(ico_current,ICO_CURRENT),
};
/*lint +e665*/
        ...
        struct device_attribute *attr, char *buf)
{
    ...
    case CHARGE_SYSFS_CHARGE_TERM_CURR_SETTING:
        return snprintf(buf, PAGE_SIZE, "%d\n", di->core_data->iterm);
    case CHARGE_SYSFS_ICO_CURRENT:
        return snprintf(buf,PAGE_SIZE, "%d\n", di->sysfs_data.ico_current);
    default:
...
static int charge_probe(struct platform_device *pdev)
{
    ...
    di->sysfs_data.ico_current = CHARGE_CURRENT_0500_MA;
    ...
    di->charger_type = CHARGER_REMOVED;
    di->charger_source = POWER_SUPPLY_TYPE_BATTERY;
    ret = of_property_read_u32(np,"switch_chip_exist",&(di->switch_chip_exist));
    if(ret)
    {
        hwlog_info("get switch_chip_exist failed, set 1 as default value!\n");
        di->switch_chip_exist = 1;
    }
    hwlog_info("switch_chip_exist = %d\n", di->switch_chip_exist);
    charge_wake_lock();
    charge_rename_charger_type(type,di);
    charge_send_uevent(di);
    schedule_work(&di->usb_work);
