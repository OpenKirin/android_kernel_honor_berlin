...
static void mt_battery_average_method_init(int *bufferdata, unsigned int data, int *sum)
{
    int i =0;
...
static int mt_battery_average_method(int *bufferdata, unsigned int voltage)
{
    int avgdata;
...
static void voltage_base_input_current_select(struct ico_output *output,int iin_option_num,struct hi6522_device_info *di)
{
    static int index = 0;
    unsigned int iin_lim = 0;
    unsigned int state = 0;
    int ret = 0;
    di->ico_iin = output->input_current;
    ...
    if(iin_option_num == 1){
        ...
        if(di->ico_iin_lim_opt == IIN_LIMIT_OPTION_DEFAULT){
            output->input_current = di->ico_iin;
            output->ico_current = di->ico_iin;
        }else if(di->ico_iin_lim_opt == IIN_LIMIT_OPTION_3){
            output->ico_current = di->ico_iin;
        }
...
int hi6522_input_current_optimize(struct ico_input *input, struct ico_output *output)
{
    struct hi6522_device_info *di = g_hi6522_dev;
    ...
    if((!bat_exist)
        || (di->ico_support == FALSE)
        || (input->charger_type != CHARGER_TYPE_STANDARD)){
        return -1;
    }

    avg_voltage = mt_battery_average_method(&batteryVoltageBuffer[0],bat_voltage);
    SCHARGER_INF("[%s],average voltage is %d\n", __func__, avg_voltage);
    if(avg_voltage < DPM_CHECK_VOLTAGE){
        if(di->ico_en){
            voltage_base_input_current_select(output,1,di);
        ...
        if(ico_hig_thr_count >= ICO_HIG_THR_COUNT_MAX){
            ico_hig_thr_count = 0;
            voltage_base_input_current_select(output,0,di);
            di->ico_en = TRUE;
        }
    }
    return 0;
					   ...
					   unsigned long event, void *data)
{
	...
	case CHARGER_TYPE_UNKNOWN:
		di->ico_en = TRUE;
		batteryBufferFirst= 1;//enable batt_buffer refresh
		schedule_delayed_work(&di->hi6522_dpm_check_work,
				      msecs_to_jiffies(0));
		break;
	case CHARGER_TYPE_NONE:
		...
		di->ico_iin = IIN_LIMIT_2A;
		ico_hig_thr_count = 0;
		batteryBufferFirst = 1;//reset flag of batt_buffer refresh
