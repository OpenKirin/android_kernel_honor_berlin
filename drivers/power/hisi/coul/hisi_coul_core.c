...
void basp_record_fcc(struct smartstar_coul_device *di)
{
    ...
    dsm_ret = charger_dsm_report(ERROR_SAFE_PLOICY_LEARN, &di->fcc_real_mah);
    if(0 != dsm_ret){
        hwlog_debug("ERROR_SAFE_PLOICY_LEARN DSM report ERROR!\n");
    }
    memcpy(&my_nv_info, pinfo, sizeof(*pinfo));
}
...
{
    int i = 0;
    unsigned int records_cnt = 0, records_sum = 0, records_avg = 0;
...
static void coul_core_get_dts(struct smartstar_coul_device *di)
{
...

    {
        ...
        for (i = 0; i < BASP_LEVEL_CNT; i++) {
            ...
            basp_policy[i].level = (BASP_LEVEL_TYPE)(one_policy[POLICY_MEM_LEVEL]);
