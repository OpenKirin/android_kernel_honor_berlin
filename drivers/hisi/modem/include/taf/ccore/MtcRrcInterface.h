...
#define  MTC_RRC_CDMA_MAX_ARFCN_NUM                         (10)                /* CDMA频点列表最大个数 */
...
/*******************************************************************************
 结构名    : RRC_MTC_CDMA_FREQ_INFO_STRU
 结构说明  : Cdma的频点信息
...
}RRC_MTC_CDMA_FREQ_INFO_STRU;

/*******************************************************************************
 结构名    : RRC_MTC_1X_FREQ_INFO_LIST_STRU
 结构说明  : 1x频点信息列表
...
typedef struct
{
    VOS_UINT8                           ucNum;                /* 频点数量，没有服务频点和邻区频点，则为0 */
    VOS_UINT8                           aucReserved[3];
    RRC_MTC_CDMA_FREQ_INFO_STRU         astCdmaFreqInfo[MTC_RRC_CDMA_MAX_ARFCN_NUM];
                                                              /* 服务小区频点放在第一个位置，没有服务小区频点置0。
                                                                         数组下标0为服务小区频点，数组下标1-9为邻区频点。
                                                                         L主模上报的时候，1X的频率信息全置0 */
}RRC_MTC_1X_FREQ_INFO_LIST_STRU;

/*******************************************************************************
 结构名    : RRC_MTC_HRPD_FREQ_INFO_LIST_STRU
 结构说明  : Hrpd频点信息列表
...
typedef struct
{
    ...
    RRC_MTC_CDMA_FREQ_INFO_STRU         astCdmaFreqInfo[MTC_RRC_CDMA_MAX_ARFCN_NUM];
                                                              /* 服务小区频点放在第一个位置，没有服务小区频点置0。
                                                                         数组下标0为服务小区频点，数组下标1-9为邻区频点。
                                                                         GUTL下，L主模上报的时候，DO的频率信息全置0。
                                                                         CL下，L主模上报时，服务小区和L邻区信息填在已有结构stLteFreqInfo，
                                                                         DO邻区信息填在此结构中，从数组下标0开始填 */
}RRC_MTC_HRPD_FREQ_INFO_LIST_STRU;

/*******************************************************************************
 结构名    : RRC_MTC_CDMA_BUSINESS_STATE_STRU
 结构说明  : Cdma的业务状态
...
typedef struct
{
    PS_BOOL_ENUM_UINT8                  en1xInBusiness;         /* 1X业务状态，TRUE为业务态,FALSE为空闲态，
                                                                           CL下，L主模上报的时候，填为FALSE */
    PS_BOOL_ENUM_UINT8                  enHrpdInBusiness;       /* HRPD业务状态，TRUE为业务态,FALSE为空闲态，
                                                                           CL下，L主模上报的时候，填为FALSE */

    VOS_UINT8                           aucReserved[2];
}RRC_MTC_CDMA_BUSINESS_STATE_STRU;
...
typedef struct
{
    ...
    RRC_MTC_1X_FREQ_INFO_LIST_STRU      st1xFreqInfo;                      /* 1x频点列表信息 */

    RRC_MTC_HRPD_FREQ_INFO_LIST_STRU    stHrpdFreqInfo;                    /* Hrpd频点列表信息 */

    RRC_MTC_CDMA_BUSINESS_STATE_STRU    stCdmaBusinessState;               /* Cdma业务状态 */
