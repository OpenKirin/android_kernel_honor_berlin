...
enum LMAC_LPHY_MSG_TYPE_ENUM
{
    ...
    ID_LMAC_LPHY_UL_HARQ_MAX_TX_NUM_IND   = (LPHY_ULLMAC_MSG_HDR + 0x0f),      /* _H2ASN_MsgChoice LMAC_LPHY_DSDS_RF_USED_IND_STRU */
...
/*****************************************************************************
 结构名    : LMAC_LPHY_UL_HARQ_MAX_TX_NUM_INFO_STRU
 协议表格  :
 ASN.1描述 :
 结构说明  : 上行HARQ发送达到最大次数信息
*****************************************************************************/
typedef struct
{
    VOS_UINT8                           ucHarqId;                               /*上行授权和HARQ的NACK反馈对应的上行数据发送的进程号*/
    ...
    VOS_UINT8                           ucTbIndex;
    VOS_UINT8                           ucResv;
}LMAC_LPHY_UL_HARQ_MAX_TX_NUM_INFO_STRU;

/*****************************************************************************
 结构名    : LMAC_LPHY_UL_HARQ_MAX_TX_NUM_IND_STRU
 协议表格  :
 ASN.1描述 :
 结构说明  : 上行HARQ发送达到最大次数上报
*****************************************************************************/
typedef struct
{
    LMAC_LPHY_MSG_TYPE_ENUM_UINT32          enMsgId;                            /*_H2ASN_Skip*/
    VOS_UINT16                              usTxSysFrame;                       /*上行HARQ达到最大次数的子帧号*/
    VOS_UINT16                              usTxSubFrame;                       /*上行HARQ达到最大次数的子帧号*/

    VOS_UINT8                               ucHarqNum;                          /*上行HARQ达到最次次数的数目*/
    VOS_UINT8                               aucRev[3];

    LMAC_LPHY_UL_HARQ_MAX_TX_NUM_INFO_STRU  astHarqMaxTxNumInfo[LMAC_LPHY_UL_GRANT_HARQ_MAX_NUM];                  /*上行授权和HARQ反馈信息*/
}LMAC_LPHY_UL_HARQ_MAX_TX_NUM_IND_STRU;
...
/*MAX HARQ*/
#define DEDI_MAC_PHY_UL_HARQ_MAX_TX_NUM_IND_FLAG_ADDR   (DEDI_MAC_PHY_ERR_IND_ADDR+sizeof(LMAC_LPHY_ERR_IND_STRU))
#define DEDI_MAC_PHY_UL_HARQ_MAX_TX_NUM_IND_ADDR        (DEDI_MAC_PHY_UL_HARQ_MAX_TX_NUM_IND_FLAG_ADDR + DEDI_MAILBOX_FLAG)

/*DSP检查到MAC PDU Valid标志为无效,把该标志置为1，MAC负责清除*/
/*#define DEDI_MAC_PDU_INVALID_FLAG_ADDR   ()*/
#define DEDI_MAC_PDU_INVALID_FLAG        (DEDI_MAC_PHY_UL_HARQ_MAX_TX_NUM_IND_ADDR + sizeof(LMAC_LPHY_UL_HARQ_MAX_TX_NUM_IND_STRU))
