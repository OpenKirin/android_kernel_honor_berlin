...
#define IMSA_IMS_NONCID_SCHEME_URI_LEN 128
#define IMSA_IMS_CID_SCHEME_URI_LEN 128
#define IMSA_IMS_CIVICLOC_COUNTRY_LEN  4
#define IMSA_IMS_CIVICLOC_COUNTRY_A1_LEN  50
#define IMSA_IMS_CIVICLOC_COUNTRY_A2_LEN  50
#define IMSA_IMS_CIVICLOC_COUNTRY_A3_LEN  50
#define IMSA_IMS_CIVICLOC_COUNTRY_A4_LEN  50
#define IMSA_IMS_CIVICLOC_COUNTRY_A5_LEN  50
#define IMSA_IMS_CIVICLOC_ROADSTREET_LEN  100
#define IMSA_IMS_CIVICLOC_OTHER_ADDRESS_LEN  100
#define IMSA_IMS_COORDINATE_LATITUDE_LEN  16
#define IMSA_IMS_COORDINATE_LONGITUDE_LEN 16
#define IMSA_IMS_COORDINATE_ALTITUDE_LEN  16
#define IMSA_IMS_TIMESTAMP_LEN              24
...
#define IMSA_IMS_DIALOGID_MAX_NUM             (7)

#define IMSA_IMS_RETRYAFTER_INVALID_VALUE   (0xFFFFFFFF)
/*****************************************************************************
  3 Massage Declare
*****************************************************************************/
...
enum IMSA_IMS_INPUT_CALL_REASON_ENUM{
    ...
    IMSA_IMS_INPUT_CALL_REASON_RESOURCE_ONLY_VOICE_READY_WAIT_VIDEO,
...
enum IMAS_IMS_INPUT_PARA_REASON_ENUM{
    ...
    IMAS_IMS_INPUT_PARA_REASON_SET_LOCATION,
...
enum IMAS_IMS_LOCATION_SCHEME_TYPE_ENUM
{
    IMAS_IMS_LOCATION_NONCID_SCHEME_TYPE = 0,
    IMAS_IMS_LOCATION_CID_SCHEME_TYPE     = 1,
    IMAS_IMS_LOCATION_TYPE_BUTT
};
...
enum IMSA_IMS_OUTPUT_SEVICE_REASON_ENUM{
    ...
    IMSA_IMS_OUTPUT_SEVICE_REASON_SUBSCRIBE_FAILED, /* 订阅错误上报 */
...
enum IMSA_IMS_INT_ERROR_CODE_ENUM
{
    ...
    IMSA_IMS_INT_ERROR_CODE_TRANS_FAIL,
...
typedef struct {
    ...
    VOS_UINT8                               aucRsv[1];
    VOS_UINT32                              ulDlgId[IMSA_IMS_DIALOGID_MAX_NUM]; /*Added for forking, indicates the dialog id,
                                                                                 取值范围: UINT16 < ulDlgId < UINT32 */
...
typedef struct{
    ...
    union {
        ...
        struct
        {
            VOS_UINT32   ulSchemeType;/*IMAS_IMS_LOCATION_SCHEME_TYPE_ENUM*/
            union
            {
                struct
                {
                    VOS_CHAR acUri[IMSA_IMS_NONCID_SCHEME_URI_LEN+1];
                    VOS_CHAR acRsv[3];
                }stNonCidScheme;
                struct
                {
                    VOS_UINT8 bitOpCivicLoc:1;
                    VOS_UINT8 bitOpCoordinateLoc:1;
                    VOS_UINT8 ucRsv:6;
                    VOS_UINT8 aucRsv[3];
                    struct
                    {
                        VOS_CHAR acUri[IMSA_IMS_CID_SCHEME_URI_LEN+1];
                        VOS_CHAR acCountry[IMSA_IMS_CIVICLOC_COUNTRY_LEN+1];
                        VOS_CHAR acCountryA1[IMSA_IMS_CIVICLOC_COUNTRY_A1_LEN+1];
                        VOS_CHAR acCountryA2[IMSA_IMS_CIVICLOC_COUNTRY_A2_LEN+1];
                        VOS_CHAR acCountryA3[IMSA_IMS_CIVICLOC_COUNTRY_A3_LEN+1];
                        VOS_CHAR acCountryA4[IMSA_IMS_CIVICLOC_COUNTRY_A4_LEN+1];
                        VOS_CHAR acCountryA5[IMSA_IMS_CIVICLOC_COUNTRY_A5_LEN+1];
                        VOS_CHAR acRoadStreet[IMSA_IMS_CIVICLOC_ROADSTREET_LEN+1];
                        VOS_CHAR acOtherAddress[IMSA_IMS_CIVICLOC_OTHER_ADDRESS_LEN+1];
                        VOS_CHAR acRsv[1];
                    }stCivicLocInfo;
                    struct
                    {
                        VOS_CHAR acLatitude[IMSA_IMS_COORDINATE_LATITUDE_LEN+1];
                        VOS_CHAR acLongitude[IMSA_IMS_COORDINATE_LONGITUDE_LEN+1];
                        VOS_CHAR acAltitude[IMSA_IMS_COORDINATE_ALTITUDE_LEN+1];
                        VOS_CHAR acRsv[1];
                    }stCoordinateLocInfo;
                    VOS_CHAR acRetentionExpiry[IMSA_IMS_TIMESTAMP_LEN+1];
                    VOS_CHAR acTimeStamp[IMSA_IMS_TIMESTAMP_LEN+1];
                    VOS_CHAR acRsv[2];
                }stCidScheme;
            }uSchemeType;
        }stLocationInfo;
...
typedef struct{
    ...
    VOS_CHAR        acReason[128];   /* IM CN subsystem XML body
                                      ...
                                      *         <reason>
                                      */
    VOS_UINT32      ulRetryAfter;    /* the retry-after header in SIP Oxx message,单位为s,0xFFFFFFFF表示没带retry-after，其他值均为从retry-after头域获取的值 */
    ...
    VOS_UINT8       ucForbidenFlag; /* 永久被禁标识，仅在周期注册，且原因值为400,402,421,484时有效 */
    VOS_UINT8       ucImsCoreOutageFlag;  /* 网侧回复#503错误时，是否携带ImsCoreOutage的标识 */
...
typedef struct {
    ...
    VOS_UINT32                             ulDlgId;     /* Indicates the dialog id, 上报资源预留请求时取值范围为UINT16 < ulDlgId < UINT32
                                                           上报其他呼叫状态时ulDlgId为0 */
...
/*****************************************************************************
 结构名    : IMSA_IMS_CALL_REMOTE_IP_LOCAL_PORT_STRU
 结构说明  : IMS在上报INITIALING EVENT时，填上remote ip和local port
*****************************************************************************/
typedef struct {
    VOS_CHAR                               acRemoteAddress[IMSA_IMS_IP_ADDRESS_STRING_SZ+1]; //远端语音承载IP地址
    VOS_UINT8                              aucRsv[1];
    VOS_UINT32                             ulLocalPort; //本地语音端口号
}IMSA_IMS_CALL_REMOTE_IP_LOCAL_PORT_STRU;
...
typedef struct {
    ...
    IMSA_IMS_CALL_REMOTE_IP_LOCAL_PORT_STRU      stCallIpPortInfo; //只在initialing event时，填上改STRUC
