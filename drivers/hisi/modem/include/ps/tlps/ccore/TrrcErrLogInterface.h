...
enum TRRC_ERRORLOG_ALARM_ID_ENUM
{
    ...
    TRRC_ERRORLOG_ALARM_ID_COLLECT_TDS_FREQ_REPORT,                             /* 收集TDS频点信息上报 */
...
enum TRRC_ERRORLOG_ALARM_TYPE_ENUM
{
    ...
    TRRC_ERRORLOG_ALARM_TYPE_MODEM_ERROR               = 6 ,      /* 表示Modem直通 */
...
enum TRRC_APP_ERROR_CODE_ENUM
{
    ...
    TRRC_APP_CS_NOFLOW_OUTOFSYNC                  = 90, /*TRRC CS域打电话时，由于没有收到或者发送成功某个流程，导致后来UE发生了失步流程*/
    TRRC_APP_CS_OUTOFSYNC_NOFLOW                  = 91, /*TRRC CS域打电话时，由于UE先发生了失步流程，导致没有收到或者发送成功某个流程*/
    TRRC_APP_CS_NOFLOW_RRCCONNREL                 = 92, /*TRRC CS域打电话时，UE在一段时间内 没有收到或者发送成功某个流程，然后网侧下发了RRC CONN REL*/
    TRRC_APP_CS_NOFLOW_SIGNALCONNREL              = 93, /*TRRC CS域打电话时，UE在一段时间内 没有收到或者发送成功某个流程P，然后网侧下发了SIGNAL CONN REL*/
    TRRC_APP_CS_NOFLOW_GURELREQ                   = 94, /*TRRC CS域打电话时，UE在一段时间内 没有收到或者发送成功某个流程，然后GU 下发了REL REQ*/
    TRRC_APP_CS_NOFLOW_GUTRANSNTFY                = 95, /*TRRC CS域打电话时，UE在一段时间内 没有收到或者发送成功某个流程，然后GU 下发了TRANS STATUS NOTIFY，并且携带的原因值为TRANS_FAIL*/
    TRRC_APP_CS_NOFLOW_OTHER                      = 96, /*TRRC CS域打电话时，UE在一段时间内 没有收到或者发送成功某个流程，其他的ERR CODE 原因值*/
    ...
    TRRC_APP_COLLECT_TDS_FREQ_REPORT              = 103,                        /* 收集TDS频点信息上报 */
    TRRC_APP_NOT_TO_EUTRA                         = 103,/*长时间不回4G*/
...
/*****************************************************************************
Description    : TRRC直通上报的CS域打电话RRC CONN REQ ERR LOG信息内容。
修改历史 :
 1.日    期    : 2016年8月23日
   作    者    : l00297133
   修改内容    : 新建结构
...
typedef struct
{
    ...
    VOS_UINT32                                         aulEstSignallingType[2];/* 域建链状态信息*/
    VOS_UINT32                                         ulCsCallChrFlowBitmap;
    VOS_UINT32                                         ulCsCallChrCodeBitmap;
...
/*****************************************************************************
Description    : TRRC本地保存的CS域打电话的PLMN、LAC、RAC信息内容。
修改历史 :
 1.日    期    : 2016年8月2日
   作    者    : l00297133
   修改内容    : 新建结构
...
typedef struct
{
    ...
    VOS_UINT32                                         ulMnc;                  /* 当前所在PLMN的Mnc*/
    VOS_UINT16                                         usLac;                  /* 当前所在的Lac*/
    VOS_UINT8                                          ucRac;                  /* 当前所在的Rac*/
    VOS_UINT8                                          ucResv;                 /* 保留位*/
}TRRC_CS_CALL_MM_INFO_STRU;
/*****************************************************************************
Description    : TRRC本地保存的CS域打电话空口消息异常的 ERR LOG信息内容。
修改历史 :
 1.日    期    : 2016年8月2日
   作    者    : l00297133
   修改内容    : 新建结构
...
typedef struct
{
    TRRC_APP_STATE_INFO_STRU                           stRrcAppStateInfo;      /* 发生故障时状态信息 */
    TRRC_APP_CS_RELEASE_CELL_INFO_STRU                 stServCellInfo;         /* 服务小区信息 */
    TRRC_CS_CALL_MM_INFO_STRU                          stMmInfo;               /* PLMN、LAC、RAC信息*/
    VOS_UINT32                                         ulTimeStamp;            /* 记录流程执行到哪一步的时间戳*/
    VOS_UINT32                                         ulCsCallChrFlowBitmap;
    VOS_UINT32                                         ulCsCallChrCodeBitmap;
...
/*****************************************************************************
Description    : TRRC本地保存的CS域打电话RRC CONN SETUP ERR LOG信息内容。
修改历史 :
 1.日    期    : 2016年6月17日
   作    者    : l00297133
   修改内容    : 新建结构
...
/*****************************************************************************
Description    : TRRC本地保存的CS域打电话RRC CONN SETUP COMP ERR LOG信息内容。
修改历史 :
 1.日    期    : 2016年7月7日
   作    者    : l00297133
   修改内容    : 新建结构
...
/*****************************************************************************
Description    : TRRC本地保存的CS域打电话Initial DirTransfer ERR LOG信息内容。
修改历史 :
 1.日    期    : 2016年7月7日
   作    者    : l00297133
   修改内容    : 新建结构
...
/*****************************************************************************
Description    : TRRC本地保存的CS域打电话SMC ERR LOG信息内容。
修改历史 :
 1.日    期    : 2016年7月7日
   作    者    : l00297133
   修改内容    : 新建结构
...
/*****************************************************************************
Description    : TRRC本地保存的CS域打电话RB SETUP ERR LOG信息内容。
修改历史 :
 1.日    期    : 2016年7月7日
   作    者    : l00297133
   修改内容    : 新建结构
...
/*****************************************************************************
Description    : TRRC直通上报的CS域打电话EST CNF ERR LOG信息内容。
修改历史 :
 1.日    期    : 2016年8月20日
   作    者    : l00297133
   修改内容    : 新建结构
...
typedef struct
{
    ...
    VOS_UINT32                                          ulCsCallChrCodeBitmap;
    VOS_UINT32                                          ulOpId;                  /* 操作标识 */
    VOS_UINT32                                          aulEstSignallingType[2]; /* 域建连状态信息*/
...
typedef struct
{
    ...
    TRRC_CHR_CS_CALL_CONN_SETUP_INFO_STRU                stCsCallConnSetupInfo;    /* 打电话流程走到CONN SETUP时本地保存的CHR信息 */
    TRRC_CHR_CS_CALL_CONN_SETUP_COMP_INFO_STRU           stCsCallConnSetupCompInfo;/* 打电话流程走到CONN SETUP COMP时本地保存的CHR信息 */
    TRRC_CHR_CS_CALL_INIT_DIRTRAN_INFO_STRU              stCsCallInitDirTranInfo;  /*打电话流程走到InitialDirTransfer时本地保存的CHR信息*/
    TRRC_CHR_CS_CALL_SMC_INFO_STRU                       stCsCallSmcInfo;        /*打电话流程走到SMC时本地保存的CHR信息*/
    ...
    TRRC_CHR_CS_CALL_RB_SETUP_INFO_STRU                  stCsCallRBSetupInfo;    /* 打电话流程走到RB SETUP时本地保存的CHR信息*/
...
/*****************************************************************************
Description    : TRRC本地保存的CS域打电话的过程中先发生失步然后走CELL UPDATE流程的有关信息
修改历史 :
 1.日    期    : 2016年2月19日
   作    者    : l00280875
   修改内容    : 新建结构
...
/*****************************************************************************
Description    : TRRC失步过程中，TRRC上报给CHR的信息内容。
                 usAsyMsgBitmap中每个BIT的含义如下:
                 BIT1~BIT5:   表示失步的原因是T312还是T313定时器超时(0:T312定时器超时，1:T313定时器超时)
                 BIT6:        网侧配置的1G事件中hysteresis的值是否合理(0:不合理，1:合理)
                 BIT7:        网侧配置的1G事件中timeToTrigger的值是否合理(0:不合理，1:合理)
                 BIT8:        网侧配置的2A事件中usedFreqW的值是否合理(0:不合理，1:合理)
                 BIT9:        网侧配置的2A事件中hysteresis的值是否合理(0:不合理，1:合理)
                 BIT10:       网侧配置的2A事件中timeToTrigger的值是否合理(0:不合理，1:合理)
                 BIT11:       网侧配置的3A事件中thresholdOwnSystem的值是否合理(0:不合理，1:合理)
                 BIT12:       网侧配置的3A事件中thresholdOtherSystem的值是否合理(0:不合理，1:合理)
                 BIT13:       网侧配置的3A事件中hysteresis的值是否合理(0:不合理，1:合理)
                 BIT14:       网侧配置的3A事件中timeToTrigger的值是否合理(0:不合理，1:合理)
                 BIT15:       网侧配置的3C事件中thresholdOtherSystem的值是否合理(0:不合理，1:合理)
                 BIT16:       网侧配置的3C事件中hysteresis的值是否合理(0:不合理，1:合理)
                 BIT17:       网侧配置的3C事件中timeToTrigger的值是否合理(0:不合理，1:合理)
                 BIT18~BIT32: 保留位

                 usAsyAssistMsgBitmap中每个BIT的含义如下:
                 BIT1~BIT5:   表示是否发生了T312还是T313定时器超时(1:发生了，0:没有发生)
                 BIT6:        表示网侧是否配置了1G事件中hysteresis的值(1:网侧配置了该参数，0:网侧没有配置该参数)
                 BIT7:        表示网侧是否配置了1G事件中timeToTrigger的值(1:网侧配置了该参数，0:网侧没有配置该参数)
                 BIT8:        表示网侧是否配置了2A事件中usedFreqW的值(1:网侧配置了该参数，0:网侧没有配置该参数)
                 BIT9:        表示网侧是否配置了2A事件中hysteresis的值(1:网侧配置了该参数，0:网侧没有配置该参数)
                 BIT10:       表示网侧是否配置了2A事件中timeToTrigger的值(1:网侧配置了该参数，0:网侧没有配置该参数)
                 BIT11:       表示网侧是否配置了3A事件中thresholdOwnSystem的值(1:网侧配置了该参数，0:网侧没有配置该参数)
                 BIT12:       表示网侧是否配置了3A事件中thresholdOtherSystem的值(1:网侧配置了该参数，0:网侧没有配置该参数)
                 BIT13:       表示网侧是否配置了3A事件中hysteresis的值(1:网侧配置了该参数，0:网侧没有配置该参数)
                 BIT14:       表示网侧是否配置了3A事件中timeToTrigger的值(1:网侧配置了该参数，0:网侧没有配置该参数)
                 BIT15:       表示网侧是否配置了3C事件中thresholdOtherSystem的值(1:网侧配置了该参数，0:网侧没有配置该参数)
                 BIT16:       表示网侧是否配置了3C事件中hysteresis的值(1:网侧配置了该参数，0:网侧没有配置该参数)
                 BIT17:       表示网侧是否配置了3C事件中timeToTrigger的值(1:网侧配置了该参数，0:网侧没有配置该参数)
                 BIT18~BIT32: 保留位
修改历史 :
 1.日    期    : 2016年2月19日
   作    者    : l00280875
...
/*****************************************************************************
Description    : 一般场景下，TRRC先发生了失步流程，然后又走了CELL UPDATE流程。这种场景下TRRC
                 上报的CHR内容。
修改历史 :
 1.日    期    : 2016年2月19日
   作    者    : l00280875
...
/*****************************************************************************
Description    : CS域TRRC先发生了失步流程，然后又走了CELL UPDATE流程。这种场景下TRRC
                 上报的CHR内容。
修改历史 :
 1.日    期    : 2016年2月19日
   作    者    : l00280875
...
/*****************************************************************************
 结构名    : TRRC_APP_COLLECT_TDS_FREQ_REPORT_STRU
 结构说明  : 收集的TDS频点信息结构
*****************************************************************************/
typedef struct
{
    VOS_UINT16                                 aulFreqList[MAX_TDS_FREQ_NUM];   /* 收集的TDS频点信息 */
...
#define MAX_CELL_RECORD_SIZE (10)        /* 最多记录最近驻留的10个T小区 */
#define MAX_FAIL_COUNT (5)               /* 最多记录最近5次发生的连接态互操作失败 */

/*****************************************************************************
 枚举名    : TRRC_APP_TL_RESEL_FALI_CAUSE_ENUM
 协议表格  :
 ASN.1描述 :
 枚举说明  : TL重选失败原因枚举
*****************************************************************************/
enum TRRC_APP_TL_RESEL_FALI_CAUSE_ENUM
{
    ...
    TRRC_APP_TL_RESEL_SIB_RCV_FAIL           = LRRC_GURRC_CELL_RESEL_SIB_RCV_FAIL,         /* 系统消息接收失败 */
    TRRC_APP_TL_RESEL_CELL_BARRED            = LRRC_GURRC_CELL_RESEL_CELL_BARRED,         /* 小区BAR */
    TRRC_APP_TL_RESEL_CELL_FORBID            = LRRC_GURRC_CELL_RESEL_CELL_FORBID,         /* 小区FORBID */
    TRRC_APP_TL_RESEL_NOT_EPLMNS             = LRRC_GURRC_CELL_RESEL_NOT_EPLMNS,         /* 小区PLMN不属于RPLMN和EPLMNS */
    TRRC_APP_TL_RESEL_ACCESS                 = LRRC_GURRC_CELL_RESEL_ACCESS,         /* 小区受限 */
    TRRC_APP_TL_RESEL_UNSPECIFIC             = LRRC_GURRC_CELL_RESEL_UNSPECIFIC,         /* 其他未定义错误 */
    TRRC_APP_TL_RESEL_CONNECTION_REJECT      = LRRC_GURRC_CELL_RESEL_CONNECTION_REJECT,         /* 立即指派被拒或连接被拒 */
    TRRC_APP_TL_RESEL_S_OR_C1_CHECK_FAIL     = LRRC_GURRC_CELL_RESEL_S_OR_C1_CHECK_FAIL,         /* 无效门限 */
    TRRC_APP_TL_RESEL_ARFCN_NOT_SUPPORT      = LRRC_GURRC_CELL_RESEL_ARFCN_NOT_SUPPORT,         /* MFBI不支持的频点 */
    TRRC_APP_TL_RESEL_RESOURCE_OCCUPIED      = 0x10,         /* 被抢占 */
    TRRC_APP_TL_RESEL_FAIL_CAUSE_BUTT
};
typedef VOS_UINT8  TRRC_APP_TL_RESEL_FALI_CAUSE_ENUM_UINT8;

/*****************************************************************************
 结构名    : TL_RESEL_FAIL_RECORD_STRU;
 结构说明  : 记录重选失败的结构体
*****************************************************************************/
typedef struct
{
    VOS_UINT8                                     ucFailCount;/* 重选失败次数 */
    TRRC_APP_TL_RESEL_FALI_CAUSE_ENUM_UINT8       enFailCause;/* 重选失败原因 */
    VOS_UINT8                                     aucReserved[2];
}TL_RESEL_FAIL_RECORD_STRU;

/*****************************************************************************
 结构名    : TRRC_EUTRA_INFO_STRU;
 结构说明  : L小区最好能量及重选门限
*****************************************************************************/
typedef struct
{
    VOS_UINT16                          usArfcn;/* L频点 */
    VOS_UINT16                          usCellId;/* L小区ID */
    VOS_INT16                           sRsrp;/* L能量值 */
    VOS_INT16                           sRsrpThreshOfEutraForResel;/* 重选L能量值门限 */
    VOS_INT16                           sRsrq;/* L信号质量 */
    VOS_INT16                           sRsrqThreshOfEutraForResel;/* 重选L信号质量门限 */
}TRRC_EUTRA_INFO_STRU;

/*****************************************************************************
 结构名    : TRRC_LOW_EUTRA_INFO_STRU;
 结构说明  : TL低优先级重选L小区最好能量及重选门限
*****************************************************************************/
typedef struct
{
    TRRC_EUTRA_INFO_STRU                stEutraInfoForResel;
    VOS_INT16                           sThreshOfServingForResel;/* 重选T门限值 */
    VOS_INT16                           sThreshOfServingForMeas;/* T起测L门限值 */
}TRRC_LOW_PRIOR_EUTRA_INFO_STRU;

/*****************************************************************************
 结构名    : TRRC_APP_CELL_INFO_STRU;
 结构说明  : 上报的TDS小区信息结构体
*****************************************************************************/
typedef struct
{
    VOS_UINT32                          ulTimeStampHigh;/* 时间戳高32位 */
    VOS_UINT32                          ulTimeStampLow;/* 时间戳低32位 */
    VOS_UINT32                          ulmcc;              /* 当前PLMN */
    VOS_UINT32                          ulmnc;              /* 当前PLMN */
    VOS_UINT32                          ulCellIdentity;     /* Globale Cell ID */
    VOS_UINT16                          usLac;              /* lac */
    VOS_UINT8                           ucRac;              /* rac */
    TRRC_APP_PROTOCOL_STATE_ENUM_UINT8  enProtocalState;    /* 协议状态 */
    VOS_UINT16                          usCellParameterID;  /* 物理小区ID */
    VOS_UINT16                          usFrequency;        /* 频点 */
    TDS_APP_FREQ_BAND_LIST_ENUM_UINT8   enBand;             /* band信息 */
    VOS_INT8                            cRscp;              /* 服务小区能量 */
    VOS_UINT8                           ucTLReselFailCount; /* 重选失败次数 */
    TRRC_APP_TL_RESEL_FALI_CAUSE_ENUM_UINT8       enTLReselFailCause;/* 重选失败原因 */
    PS_BOOL_ENUM_UINT8                  enEqualPriorFlag;   /* 是否存在等优先级LTE邻区 */
    PS_BOOL_ENUM_UINT8                  enHighPriorFlag;    /* 是否存在高优先级LTE邻区 */
    PS_BOOL_ENUM_UINT8                  enLowPriorFlag;     /* 是否存在低优先级LTE邻区 */
    VOS_UINT8                           ucReserved;
    TRRC_EUTRA_INFO_STRU                stHighPriorEutraNerghborInfo; /* 高优先级L邻区测量值及门限 */
    TRRC_LOW_PRIOR_EUTRA_INFO_STRU      stLowPriorEutraNerghborInfo; /* 低优先级L邻区测量值及门限 */
 }TRRC_APP_CELL_INFO_STRU;

/*****************************************************************************
 结构名    : TRRC_APP_TL_HO_FAIL_CAUSE_ENUM _UINT8;
 结构说明  : 切换失败原因枚举
*****************************************************************************/
enum TRRC_APP_TL_HO_FAIL_CAUSE_ENUM
{
    TRRC_APP_TL_HO_WAIT_CNF_EXPIRED       = 0x00,
    TRRC_APP_TL_HO_CONF_UNACCEPTABLE      = LRRC_GURRC_HANDOVER_RESULT_CONF_UNACCEPTABLE,         /* 不支持配置 */
    TRRC_APP_TL_HO_PHY_CHANNEL_FAIL       = LRRC_GURRC_HANDOVER_RESULT_PHY_CHANNEL_FAIL,         /* 物理信道失败 */
    TRRC_APP_TL_HO_IRAT_PROTOCOL_ERR      = LRRC_GURRC_HANDOVER_RESULT_IRAT_PROTOCOL_ERR,         /* 协议错误 */
    TRRC_APP_TL_HO_UNSPECIFIC             = LRRC_GURRC_HANDOVER_RESULT_UNSPECIFIC,         /* 未定义错误 */
    TRRC_APP_TL_HO_ARFCN_NOT_SUPPORT      = LRRC_GURRC_HANDOVER_RESULT_ARFCN_NOT_SUPPORT,         /* 不支持频点 */
    TRRC_APP_TL_HO_RESOURCE_OCCUPIED      = 0x06,         /* 被抢占 */
    TRRC_APP_TL_HO_FAIL_CAUSE_BUTT
};
typedef VOS_UINT8  TRRC_APP_TL_HO_FAIL_CAUSE_ENUM_UINT8;

/*****************************************************************************
 结构名    : TL_HO_FAIL_INFO_STRU;
 结构说明  : 记录切换失败的结构体
*****************************************************************************/
typedef struct
{
    VOS_UINT32                                   ulTimeStampHigh;/* 时间戳高32位 */
    VOS_UINT32                                   ulTimeStampLow;/* 时间戳低32位 */
    TRRC_APP_TL_HO_FAIL_CAUSE_ENUM_UINT8         enFailCause; /* 失败原因 */
    VOS_UINT8                                    aucReserved[3];
}TL_HO_FAIL_INFO_STRU;

/*****************************************************************************
 结构名    : TRRC_APP_TL_REDIR_FAIL_CAUSE_ENUM _UINT8;
 结构说明  : 重定向失败原因枚举
*****************************************************************************/
enum TRRC_APP_TL_REDIR_FAIL_CAUSE_ENUM
{
    TRRC_APP_TL_REDIR_WAIT_CNF_EXPIRED     = 0x00,
    TRRC_APP_TL_REDIR_FAIL                 = LRRC_GURRC_REDIRECTED_FAIL,         /* L回复重定向失败 */
    TRRC_APP_TL_REDIR_ARFCN_NOT_SUPPORT    = LRRC_GURRC_REDIRECTED_ARFCN_NOT_SUPPORT,         /* 不支持频点 */
    TRRC_APP_TL_REDIR_RESOURCE_OCCUPIED    = 0x03,         /* 被抢占 */
    TRRC_APP_TL_REDIR_FAIL_CAUSE_BUTT
};
typedef VOS_UINT8  TRRC_APP_TL_REDIR_FAIL_CAUSE_ENUM_UINT8;

/*****************************************************************************
 结构名    : TL_REDIR_FAIL_INFO_STRU;
 结构说明  : 记录切换失败的结构体
*****************************************************************************/
typedef struct
{
    VOS_UINT32                                   ulTimeStampHigh;/* 时间戳高32位 */
    VOS_UINT32                                   ulTimeStampLow;/* 时间戳低32位 */
    TRRC_APP_TL_REDIR_FAIL_CAUSE_ENUM_UINT8      enFailCause; /*失败原因*/
    VOS_UINT8                                    aucReserved[3];
}TL_REDIR_FAIL_INFO_STRU;

/*****************************************************************************
 结构名    : TRRC_APP_NOT_TO_EUTRA_INFO_STRU;
 结构说明  : 长期不驻留4G上报的CHR结构体
*****************************************************************************/
typedef struct
{
    OM_ERR_LOG_HEADER_STRU                  stHeader;                /* 消息头 */
    TRRC_APP_ERROR_CODE_ENUM_UINT8          enErrorCode;             /* 错误码*/
    VOS_UINT8                               ucServingCellCount;      /* 记录的小区数目 */
    VOS_UINT8                               ucTLHoCount;             /* 切换失败次数 */
    VOS_UINT8                               ucTLRedirCount;          /* 重定向失败次数 */
    TRRC_APP_CELL_INFO_STRU                 astTcellInfo[MAX_CELL_RECORD_SIZE];  /* 不回4G小区记录 */
    TL_HO_FAIL_INFO_STRU                    astTLHoInfo[MAX_FAIL_COUNT];      /* TL切换失败记录 */
    TL_REDIR_FAIL_INFO_STRU                 astTLRedirInfo[MAX_FAIL_COUNT];     /* TL重定向失败记录 */
