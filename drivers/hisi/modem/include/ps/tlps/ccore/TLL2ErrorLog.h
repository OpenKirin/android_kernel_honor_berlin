...
enum TLL2_ERRLOG_ALM_TYPE_ENUM
{
    ...
    TLL2_ERRLOG_ALM_TYPE_MODEM_ERROR            = 0x06,     /* 直通处理*/
...
/*****************************************************************************
 枚举名    : TLL2_LMAC_PREAMBLE_SPEC_NOT_MATCH_RESULT_ENUM
 枚举说明  : LMAC随机接入失败preamble不匹配时的结果
*****************************************************************************/
enum TLL2_LMAC_PREAMBLE_SPEC_NOT_MATCH_RESULT_ENUM
{
    PREAMBLE_CONSECUTIVE_LESS1_AND_RA_FAIL,  /*进入到连续RAR中preamble比发送preamble小1场景,即小区半径外接入场景,但最终接入失败了 */
    PREAMBLE_CONSECUTIVE_LESS1_AND_RA_SUCC,  /*进入到连续RAR中preamble比发送preamble小1场景,即小区半径外接入场景,但最终接入成功了 */
...
/*****************************************************************************
Description    : TLL2_DIRECT_CHR_COMMON_STRU直通CHR新增公共上报数据结构模板
修改历史 :
 1.日    期    : 2016年7月27日
   作    者    : n00181501
...
/*****************************************************************************
结构名    : TLL2_LMAC_PREAMBLE_SPEC_NOT_MATCH_LOG_STRU
协议表格  :
ASN.1描述 :
结构说明  : 一项preamble不匹配记录的详细内容
*****************************************************************************/
typedef struct
{
    TLL2_LMAC_PREAMBLE_SPEC_NOT_MATCH_RESULT_ENUM_UINT8 enRaResult; /*最终的注册结果 */
    VOS_UINT8               ucRsv;
    VOS_UINT16              usSelfAdjTaTimes; /*调整TA的次数，统计发给PHY的selfAdjTaFlag的次数 */
    LRRC_OM_SCELL_ID        stCellInfo;       /*服务小区信息 */
    LMAC_OM_PRACH_CONFIG_STRU stPrachConfig;    /*prach配置参数 */
}TLL2_LMAC_PREAMBLE_SPEC_NOT_MATCH_LOG_STRU;

/*****************************************************************************
结构名    : LMAC_OM_PREAMBLE_SPEC_NOT_MATCH_INFO_STRU
协议表格  :
ASN.1描述 :
结构说明  : 报给OM的preamble不匹配数据结构
*****************************************************************************/
typedef struct
{
