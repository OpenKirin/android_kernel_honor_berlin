...
#define PHY_ERR_LOG_CHAN_SWITCH_BUF_SIZE        (4)


#define PHY_ERR_LOG_RF_UNLOCK_TX_PLL            (2)
...
enum PHY_ERR_LOG_ALM_ID_ENUM
{
    ...
    PHY_ERR_LOG_ALM_RCM_CHANNEL_SWITCH_INFO             = 0x0b,                 /* DR-DSDS 主辅通道切换信息 */
    ...
    PHY_ERR_LOG_ALM_WCDMA_RF_UNLOCK_DETECT              = 0x2006,               /* 检测到RF失锁异常的CHR */
...
typedef struct
{
    ...
    VOS_UINT16                              usGsdrFlag;                         /* 是否启动了软基带. */
...
/*****************************************************************************
 结构名    : PHY_MNTN_CHAN_SWITCH_INFO_BUF_STRU
 协议表格  :
 ASN.1描述 :
 结构说明  : 主辅通道切换可维可测信息
*****************************************************************************/
typedef struct
{
    ...
    PHY_UINT16                               uhwSwitchType;                     /* 0---- 切到翻转态 1----切回正常态 */
    PHY_UINT16                               uhwSwitchTaskId;                   /* 导致切到翻转态的业务任务ID, 切回正常态时候填0xffff */
}PHY_MNTN_CHAN_SWITCH_INFO_BUF_STRU;

/*******************************************************************************
 结构名    : PHY_ERRLOG_CHANNEL_SWITCH_INFO_STRU
 结构说明  : 最近4次主辅通道切换记录，循环覆盖存放，每次上报之后切换次数重新计数，从buffer 0开始存放，
             如果切换次数<=4,切换记录保存在buffer的0~切换次数-1，否则buffer的(切换次数-4)%4 ~ (切换次数-1)%4，
             存放最近4次切换记录
 1.日    期   : 2016年04月20日
   作    者   : w00138719
   修改内容   : 新建
...
{
    OM_ERR_LOG_HEADER_STRU                   stHeader;
    PHY_UINT16                               uhwSwitchNum;                                      /* 本次上报的切换次数 */
    PHY_UINT16                               uhwReserved;
    PHY_MNTN_CHAN_SWITCH_INFO_BUF_STRU       astSwitchInfo[PHY_ERR_LOG_CHAN_SWITCH_BUF_SIZE];   /* 上报切换buf */
}PHY_ERRLOG_CHANNEL_SWITCH_INFO_STRU;

/*****************************************************************************
 结构名    : PHY_ERRLOG_RF_RESET_UNION
 结构说明  : RF RESET 的参数
*****************************************************************************/
typedef union PHY_ERRLOG_RF_RESET_UNION
{
    PHY_UINT32                          uwResetPara;

    struct
    {
        PHY_UINT32                      RfUnlockPll :8;                         /* RX PLL1:0  RX PLL2:1  TX PLL:2 */
        PHY_UINT32                      RfIC        :8;                         /* RFIC0:0   RFIC1:1 */
        PHY_UINT32                      ReqStatus   :8;                         /* FREE:0   FACH:2 */
        PHY_UINT32                      Band        :8;
    }reg;
}PHY_ERRLOG_RF_RESET_UNION;

/*******************************************************************************
 结构名    : PHY_MNTN_WCDMA_RF_UNLOCK_EVENT_STRU
 结构说明  : W检测到失锁事件上报
 1.日    期   : 2016年07月16日
   作    者   : l00184004
   修改内容   : 新建
...
{
    OM_ERR_LOG_HEADER_STRU                      stHeader;
    PHY_ERRLOG_RF_RESET_UNION                   unRfRest;
    PHY_UINT16                                  uhwExecutCnt;                   /* 真正执行失锁次数统计 */
    PHY_UINT16                                  uhwDetectCnt;                   /* 检测到失锁次数统计 */
    PHY_UINT32                                  uwTimeSSlice;
}PHY_MNTN_WCDMA_RF_UNLOCK_EVENT_STRU;
/*****************************************************************************
  8 UNION定义
*****************************************************************************/
