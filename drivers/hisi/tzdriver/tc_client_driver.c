...
static int TC_NS_load_image(TC_NS_DEV_File *dev_file, void* argp, unsigned cmd)
{
	...
	if(copy_from_user(&reg_buf_size,
		client_context.params[0].memref.size_addr, sizeof(unsigned int))) {
	...
	k_register_buf_p = (struct reg_buf_st *)kzalloc(reg_buf_size, GFP_KERNEL);
	if(!k_register_buf_p) {
	...
	if (copy_from_user(k_register_buf_p,
		client_context.params[0].memref.buffer, reg_buf_size)) {
	...
	for(index = 0; index < reg_buf_size/sizeof(struct reg_buf_st); index++) {
		...
		shared_mem = tc_mem_allocate(dev_file, k_register_buf->file_size);
		...
		if (copy_from_user(shared_mem->kernel_addr,
			k_register_buf->file_buffer, k_register_buf->file_size)) {
		...
		/* load image opration */
		memset(operation, 0, sizeof(TC_NS_Operation));
...
static int TC_NS_need_load_image(TC_NS_DEV_File *dev_file, void *argp, unsigned cmd)
{
	int ret;
	TC_NS_SMC_CMD *smc_cmd = NULL;
	TC_NS_Operation *operation = NULL;
