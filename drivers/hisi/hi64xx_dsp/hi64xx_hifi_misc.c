...
static int hi64xx_sync_write(const unsigned char *arg, const unsigned int len)
{
	...
	while (count) {
		if (dsp_priv->is_watchdog_coming) {
			HI64XX_DSP_INFO("can not send msg because watchdog has already come\n");
			ret = BUSY;
			goto out;
		}
		...
		HI64XX_DSP_ERROR("DSP_NMI:0x%x, CMD_STAT_VLD:0x%x, CMD_STAT:0x%x\n",
			hi64xx_hifi_read_reg(HI64xx_DSP_DSP_NMI),
			hi64xx_hifi_read_reg(HI64xx_CMD_STAT_VLD),
			hi64xx_hifi_read_reg(HI64xx_CMD_STAT));
...
static bool hi64xx_hifi_notify_dsp_pllswitch(unsigned char state)
{
	...
	if (wait_event_interruptible_timeout(dsp_priv->dsp_pllswitch_wq,
		...
		HI64XX_DSP_ERROR("wait for dsp_pllswitch_done timeout, DSP_NMI:0x%x\n",
				hi64xx_hifi_read_reg(HI64xx_DSP_DSP_NMI));
	}
	return true;
}
