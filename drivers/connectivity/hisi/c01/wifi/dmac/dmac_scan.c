            ...
            if (!OAL_MEMCMP(st_mib_ssid.ac_ssid, puc_ssid, uc_ssid_len))
            {
                    ...
                    && (0 != uc_frame_channel))
                {
                    ...
                    if (!mac_is_dbac_running(pst_mac_device))
                    {
                        ...
                        OAM_WARNING_LOG3(pst_dmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_SCAN, "{dmac_scan_check_assoc_ap_channel::AP has switch channel home chan[%d], new chan[%d], new idx[%d]}\r\n",
                                       pst_mac_device->st_home_channel.uc_chan_number, uc_frame_channel, uc_idx);
