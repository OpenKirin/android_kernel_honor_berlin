...
int offset_dis = 0;
int edge_ax_limit = 0;
int start_upx_limit = 0;
int start_dnx_limit = 0;
int edge_wideth = 0;
...
int ts_algo_t3(struct ts_device_data *dev_data, struct ts_fingers *in_info, struct ts_fingers *out_info)
{
	...
	{
		local_param = &(dev_data->edge_performance_algorithm_data);
	}
	if (g_ts_data.edge_wideth != edge_stop_lt)
	{
		if(dev_data->edge_performance_algorithm_data.config_by_dts)
		{
			edge_stop_lt = local_param->edge_wideth;//default 10
			edge_stop_rt = local_param->edge_x_max - edge_stop_lt;
			edge_ax_left = edge_stop_lt + local_param->edge_ax_limit;
			edge_ax_right = edge_stop_rt - local_param->edge_ax_limit;
			start_upx_left = edge_stop_lt + local_param->start_upx_limit;
			start_upx_right = edge_stop_rt - local_param->start_upx_limit;
			start_dnx_left = edge_stop_lt + local_param->start_dnx_limit;
			start_dnx_right = edge_stop_rt - local_param->start_dnx_limit;
			edge_y_mid = local_param->edge_y_mid;
			edge_x_max = local_param->edge_x_max;
			stop_limited= local_param->stop_limited;
			y_move_s = local_param->y_move_s;
			y_move_m = local_param->y_move_m;
			center_x = local_param->center_x;
			offset_dis = local_param->offset_dis;
		}
		else
		{
			...
			edge_y_mid = EDGE_Y_MID;
			edge_x_max = EDGE_X_MAX;
			stop_limited= STOP_LIMITED;
			y_move_s = Y_MOVE_S;
			y_move_m = Y_MOVE_M;
			center_x = CENTER_X;
