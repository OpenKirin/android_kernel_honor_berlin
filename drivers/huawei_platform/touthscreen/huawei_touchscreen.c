...
void ts_edge_performance_algorithm_achieve(struct ts_device_data *chip_data){
	int retval  = NO_ERR;
	unsigned int value = 0;
	struct edge_performance_algorithm *local_param = NULL;
	...
	local_param = &(chip_data->edge_performance_algorithm_data);
	memset(local_param, 0, sizeof(struct edge_performance_algorithm));
	...
	retval = of_property_read_u32(device, EDGE_ALGORITHM_CONFIG_BY_DTS, &value);
	if (retval) {
		local_param->config_by_dts = 0;
		TS_LOG_ERR("get device config_by_dts failed\n");
	}else{
		local_param->config_by_dts = value;
	}
	if (!local_param->config_by_dts){
		TS_LOG_INFO("product do not suppurt edge_performance_algorithm, set all param to 0\n");
		return ;
	}
	retval = of_property_read_u32(device, EDGE_ALGORITHM_EDGE_Y_MID, &value);
	if (retval) {
		local_param->edge_y_mid = 0;
		TS_LOG_ERR("get device edge_y_mid failed\n");
	}else{
		local_param->edge_y_mid = value;
	}
	retval = of_property_read_u32(device, EDGE_ALGORITHM_EDGE_X_MAX, &value);
	if (retval) {
		local_param->edge_x_max = 0;
		TS_LOG_ERR("get device edge_x_max failed\n");
	}else{
		local_param->edge_x_max = value;
	}
	retval = of_property_read_u32(device, EDGE_ALGORITHM_STOP_LIMITED, &value);
	if (retval) {
		local_param->stop_limited= 0;
		TS_LOG_ERR("get device stop_limited failed\n");
	}else{
		local_param->stop_limited = value;
	}
	retval = of_property_read_u32(device, EDGE_ALGORITHM_Y_MOVE_S, &value);
	if (retval) {
		local_param->y_move_s = 0;
		TS_LOG_ERR("get device y_move_s failed\n");
	}else{
		local_param->y_move_s = value;
	}
	retval = of_property_read_u32(device, EDGE_ALGORITHM_Y_MOVE_M, &value);
	if (retval) {
		local_param->y_move_m = 0;
		TS_LOG_ERR("get device y_move_m failed\n");
	}else{
		local_param->y_move_m = value;
	}
	retval = of_property_read_u32(device, EDGE_ALGORITHM_CENTER_X, &value);
	if (retval) {
		local_param->center_x = 0;
		TS_LOG_ERR("get device center_x failed\n");
	}else{
		local_param->center_x = value;
	}
	retval = of_property_read_u32(device, EDGE_ALGORITHM_OFFSET_DIS, &value);
	if (retval) {
		local_param->offset_dis= 0;
		TS_LOG_ERR("get device offset_dis failed\n");
	}else{
		local_param->offset_dis = value;
	}
	retval = of_property_read_u32(device, EDGE_ALGORITHM_EDGE_AX_LIMIT, &value);
	if (retval) {
		local_param->edge_ax_limit = 0;
		TS_LOG_ERR("get device edge_ax_limit failed\n");
	}else{
		local_param->edge_ax_limit = value;
	}
	retval = of_property_read_u32(device, EDGE_ALGORITHM_START_UPX_LIMIT, &value);
	if (retval) {
		local_param->start_upx_limit = 0;
		TS_LOG_ERR("get device start_upx_limit failed\n");
	}else{
		local_param->start_upx_limit = value;
	}
	retval = of_property_read_u32(device, EDGE_ALGORITHM_START_DNX_LIMIT, &value);
	if (retval) {
		local_param->start_dnx_limit = 0;
		TS_LOG_ERR("get device start_dnx_limit failed\n");
	}else{
		local_param->start_dnx_limit = value;
	}
	retval = of_property_read_u32(device, EDGE_ALGORITHM_EDGE_WIDETH, &value);
	if (retval) {
		local_param->edge_wideth= 0;
		TS_LOG_ERR("get device edge_wideth failed\n");
	}else{
		local_param->edge_wideth = value;
	}
	TS_LOG_INFO("%s:"
		" config_by_dts:%d, edge_y_mid:%d, edge_x_max:%d, stop_limited:%d, y_move_s:%d, y_move_m:%d, "
		" center_x:%d offset_dis:%d, edge_ax_limit:%d, start_upx_limit:%d, start_dnx_limit:%d, edge_wideth:%d,\n",
		__func__,
		local_param->config_by_dts,
		local_param->edge_y_mid,
		local_param->edge_x_max,
		local_param->stop_limited,
		local_param->y_move_s,
		local_param->y_move_m,
		local_param->center_x,
		local_param->offset_dis,
		local_param->edge_ax_limit,
		local_param->start_upx_limit,
		local_param->start_dnx_limit,
		local_param->edge_wideth);
}
