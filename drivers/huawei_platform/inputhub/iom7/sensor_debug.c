...
extern ssize_t handpress_check(char *buf, int maxlen);
static ssize_t show_hp_check(struct device *dev,
					   struct device_attribute *attr,
					   const char *buf, size_t size)
{
	char check_buf[32] = {0x00};
	ssize_t ret = 0;

	ret = handpress_check(check_buf, sizeof(check_buf)/sizeof(check_buf[0]));

	if (ret < 0)
		return snprintf(buf, MAX_STR_SIZE, "-1\n");
	else
		return snprintf(buf, MAX_STR_SIZE, "%s\n", check_buf);
}
...
static DEVICE_ATTR(hp_check, 0440, show_hp_check, NULL);
