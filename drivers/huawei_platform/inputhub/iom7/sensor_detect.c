...
ssize_t handpress_check(char *buf, int maxlen)
{
	int i = 0;
	int ret = 0;
	int bus_num = 0;
	...
	u32 reg = 0;
	char version_addr = 0x50;
	char fw_chip[2] = {0x00};
	int fw_versions[2] = {0x00};

	for (i = 0; i < CYPRESS_CHIPS; i++) {
		reg = handpress_data.i2c_address[i];
		hwlog_info("reg:%u\n", reg);
		ret = mcu_i2c_rw(bus_num, reg, version_addr,
				  CMD_I2C_READ_REQ,
				  sizeof(fw_chip)/sizeof(fw_chip[0]),
				  fw_chip);
		if (ret < 0) {
			hwlog_err("hp check w err! ret:%d\n", ret);
			chip_check_result |= 1<<i;
			continue;
		}

		hwlog_err("fw version %2x %2x\n", fw_chip[0], fw_chip[1]);
		fw_versions[i] = (((int)fw_chip[1])<<8) | (int)fw_chip[0];
		memset(fw_chip, 0, sizeof(fw_chip));
	}

	return snprintf(buf, (maxlen-1), "%d,%d,%d\n",
			chip_check_result, fw_versions[0], fw_versions[1]);
