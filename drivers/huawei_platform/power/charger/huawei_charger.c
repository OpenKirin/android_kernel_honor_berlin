...
*  return value:  NULL
**********************************************************/
static int charge_turn_on_force_ico
(struct charge_device_info *di, unsigned int *pIdpm_limit_val)
{
	int ret = 0;
	...
	if((di->sysfs_data.support_level_charging == 1)
		...
		if(di->start_attemp_level_charging) {
			...
			if(!charge_turn_on_force_ico(di, &idpm_limit_val)) {
				iin_current_final = idpm_limit_val > iin_current_final ?
					idpm_limit_val : iin_current_final;
				ichrg_current_final = idpm_limit_val > ichrg_current_final ?
					idpm_limit_val : ichrg_current_final;
