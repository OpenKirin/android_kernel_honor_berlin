...
static irqreturn_t hw_otg_id_irq_handle(int irq, void *dev_id)
{
    ...
    if (0 == gpio_value){
    	ret = irq_set_irq_type(otg_gpio_id_dev_p->irq, IRQF_TRIGGER_HIGH | IRQF_NO_SUSPEND);
    }
	else {
		ret = irq_set_irq_type(otg_gpio_id_dev_p->irq, IRQF_TRIGGER_LOW | IRQF_NO_SUSPEND);
	}
	enable_irq(otg_gpio_id_dev_p->irq);
