...
int data)
{
    ...
    if (data == TORCH_LEFT_MODE) {
        ...
        /* REG_CURRENT_CONTROL[3:0] control flash current */
        val = ((val & 0x80) | (0x0));
    } else if (data == LED_PROMPT_MODE){
        cam_info("%s flash_led=LED_PROMPT_MODE", __func__);
        /*set led lum to least value for prompt mode*/
        i2c_func->i2c_write(i2c_client, REG_MAX_CURRENT, 0x0c);
        val = ((val & 0x80) | (0x7f));
    }else {
    ...
    enum led_brightness brightness)
{
    ...
    if (STANDBY_MODE == led_bright) {
        ...
        if (LED_PROMPT_MODE == led_bright){
            cdata.mode = LED_PROMPT_MODE;
            cdata.data = 0;
        }else {
            int max_level;
            max_level = 3;
